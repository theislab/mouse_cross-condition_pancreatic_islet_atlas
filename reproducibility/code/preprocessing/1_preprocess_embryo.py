...
path_data='/lustre/groups/ml01/workspace/karin.hrovatin/data/pancreas/scRNA/GSE132188/rev7/scanpy_AnnData/E1*/'
...
samples=metadata.query('for_reference == False')
...
# %% [markdown]
# #C: Based on plot of cell count ranks this study does not seem to have lower proportion of ambient counts (comparing height of cell and ambient plateaus). Thus it is possible that with current threshold more ambient counts are retained than in other studies.
#
# #C: It seems that individual genes contribute less to ambience than in other datasets. 
...
# %% [markdown]
# #C: There is a subset of low counts and low genes cells that have high mt_fraction. There are some regions of high n_genes but less high n counts.
...
# %%
p1 = sc.pl.scatter(adata[np.logical_and(adata.obs['n_genes']<1500, adata.obs['n_counts']<25000)], 'n_counts', 'n_genes', color='mt_frac', size=20,show=False)
p1.grid()
plt.show()

# %% [markdown]
# #C: By increasing min n_genes thersholds the high count low n genes population could be removed. However, this would remove a lot of other cells as well thus the threshold will not be increased solely to remove the few additional cells from the mentioned population.
...
#p3.set_xscale('log')
p4 = sb.distplot(adata.obs['n_counts'][adata.obs['n_counts']<6000], 
                 kde=False, bins=50, 
                 ax=fig.add_subplot(fig_ind[1]))
#p4.set_xscale('log')
p5 = sb.distplot(adata.obs['n_counts'][adata.obs['n_counts']>40000], 
                 kde=False, bins=60, 
...
# %% [markdown]
# #C: High N counts will be filtered out, but not high N genes.
