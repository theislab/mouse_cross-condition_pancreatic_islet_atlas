...
# %%
p1 = sc.pl.scatter(adata[np.logical_and(adata.obs['n_genes']<1000, adata.obs['n_counts']<20000)], 'n_counts', 'n_genes', color='mt_frac', size=20,show=False)
p1.grid()
plt.show()
...
p9 = sb.distplot(adata.obs['mt_frac'][(adata.obs['mt_frac']<0.4).values & (adata.obs['mt_frac']>0.08).values], 
...
# %% [markdown]
# #C: High N cells and N gene outliers will be filtered out as well, despite high N genes  might be specific subpopulation on UMAP - but they overlap high N counts greatly and would be filtered out anyways. Min counts will not be used for filtering as it is unclear where to put a threshold - mt and gene filtering will as it seems also remove these cells. 
