...
path_data='/lustre/groups/ml01/workspace/karin.hrovatin/data/pancreas/scRNA/GSE117770/scanpy_AnnData/SRR7610*/'
#Unique ID2 for reading/writing h5ad files with helper function
UID2='NOD_elimination_pp'
...
metadata=pd.read_excel('/lustre/groups/ml01/workspace/karin.hrovatin/data/pancreas/scRNA/scRNA-seq_pancreas_metadata.xlsx',
             sheet_name='NOD_elimination')

# %%
# Find samples used for reference
samples=metadata
print('Non-reference samples:')
print(samples)
...
ax.axvline(50000,c='k',alpha=0.5)
ax.axhline(30,c='k',alpha=0.5)

# %% [markdown]
# #C: This dataset might be problematic in terms of ambient DNA as the plateu of empty cells with ambient DNA is poorly visible. 
...
sb.heatmap(ambient_df.sort_values('mean_ambient_n_counts',ascending=False).iloc[:20,:]
           ,annot=True,fmt='.2e',yticklabels=True)
...
# %%
# Save genes with scaled mean ambient expression at least > 0.01 in any sample - 
...
ambient_genes_selection=list(ambient_df[(ambient_df>0.01).any(axis=1)].index)
...
# %% [markdown]
# #C: There is a subset of low counts and low genes cells that have high mt_fraction and subset with very high n_counts.
...
# %% [markdown]
# #C: Samples ..97 and ..96 have subpopulation of cells with higher mt proportion that is not very high. This is also slightly visible in ..95 - all of these samples are from 8 w. It is thus possible that they are of lower quality (this could also be biological effect, but in other studies cells with such mt proportion were generally removed - indeed there seem to be no higher mt but not low gene/count subpopulations).

# %%
p1 = sc.pl.scatter(adata[np.logical_and(adata.obs['n_genes']<1500, adata.obs['n_counts']<15000)], 'n_counts', 'n_genes', color='mt_frac', size=20,show=False)
p1.grid()
plt.show()
...
p7 = sb.distplot(adata.obs['n_genes'][adata.obs['n_genes']<1300], 
...
p9 = sb.distplot(adata.obs['mt_frac'][(adata.obs['mt_frac']<0.3).values & (adata.obs['mt_frac']>0.05).values], 
                 kde=False, bins=80, ax=fig.add_subplot(fig_ind[1]))
plt.show()
...
# %% [markdown]
# #C: There is no evident border for selecting low N counts cells thus this filter will not be used. Selecting lower (0.15) or higher (0.4) mt threshold removes cells close on UMAP thus making it less likely that cells with ~0.2 mt are biological subpopulation. High N counts and N genes cells will be removed.
...
sb.distplot(adata[adata.obs.doublet_score.between(0.1,0.5)].obs['doublet_score'], bins=40, kde=False,ax=ax[1])
plt.show()
