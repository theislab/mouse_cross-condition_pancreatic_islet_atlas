...
# Evaluate integration of a cell subset.
# - Plot embedding with different batch and cell type related information.
# - Evaluate integrated embedding with a subset of scIB metrics on different data subsets.
...
cell_path=str(sys.argv[5])
out_folder=str(sys.argv[6])
cell_group=str(sys.argv[7])
if len(sys.argv) > 8:
...
# all - all cells, anno - annotated cells
P_ALL='all'
P_ANNO='anno'
OMIT_PARTITIONS=[] 
...
path_save=path_out+folder+out_folder
...
if slurm:
    latent_adata=sc.read_h5ad(path_out+folder+'latent.h5ad')
else:
    latent_adata=h.open_h5ad(file=path_out+folder+'latent.h5ad',unique_id2=UID2)
...
# Subset to the desired cells
cells_sub=pickle.load(open(cell_path,'rb'))
print('Subsetting cells. N cells to subset:',len(cells_sub))
print('N cells before subset in latent adata:',latent_adata.shape[0])
# Latent data should contain all subset cells
latent_adata=latent_adata[cells_sub,:]
print('N cells after subset in latent adata:',latent_adata.shape[0])

# Recompute embeddding as cells were subsetted
...
if UPDATE_METRICS !=NM:
    # ** Compute batch metircs on all cells (including without cell types)
    #
    ...
    if P_ALL not in OMIT_PARTITIONS:
        ...
        all_metrics=['PC_regression','ASW_batch','kBET','graph_connectivity','graph_iLISI',
                    'graph_cLISI','NMI', 'ARI','ASW_cell_type','isolated_label_F1',
                     'isolated_label_ASW','moransi_conservation']
        ...
        # Omit bio metrics - require annotation, 
        # do not omit batch as they are performed on clusters
        # Also do not omit moransi_conservation as it does not need cell labels
        for metric in ['graph_cLISI', 'NMI', 'ARI', 'ASW_cell_type',
                       'isolated_label_F1', 'isolated_label_ASW']:
            metrics_omit[metric]=False 
            metrics_omit_study[metric]=False 
        ...
        if len(metrics_to_compute)>0 or len(metrics_to_compute_study)>0: 
            ...
            na_cells=latent_adata_temp.obs_names[
                latent_adata_temp.obs[cell_group].str.contains('NA|multiplet')]
            print('N cells with NA or multiplet annotation:',len(na_cells))
            latent_adata_temp.obs.loc[na_cells,cell_group]=np.nan
            
            print('Data shapes: ref',adata_full.shape,'latent',latent_adata_temp.shape)
            ...
            compute_metrics(adata_full=adata_full,latent_adata=latent_adata_temp,metrics=metrics,
                            cell_type_col=cell_group, batch_col='study_sample',
                            path_save=path_save,
                            batch_anno_clusters=True,
                            task_name='all_allCells',TMP=TMP,
                            **metrics_omit)
            compute_metrics(adata_full=adata_full,latent_adata=latent_adata_temp,metrics=metrics_study,
                            cell_type_col=cell_group, batch_col='study', 
                            path_save=path_save,
                            batch_anno_clusters=True,
                            task_name='all_allCells_study',TMP=TMP,
                            **metrics_omit_study)
...
if UPDATE_METRICS !=NM:  
    # ** Compute bio metrics on ref cells that are not NA/multiplet
    if P_ANNO not in OMIT_PARTITIONS:
        ...
        latent_adata=latent_adata[[obs for obs in adata_ref.obs_names 
                                   if obs in latent_adata.obs_names],:]
        if n_cells_all > latent_adata.shape[0]:
            ...
            print('N all cels %i, N cells for score evaluation (shared with ref) %i'%
                  (n_cells_all,latent_adata.shape[0]))

        # Remove cells that contain NA or multiplet in their name (in cell_grop col) 
        # from the evaluation.

        # Remove cells not ok for evaluation
        eval_cells=~latent_adata.obs[cell_group].str.contains('NA|multiplet')
        print('All cells:',latent_adata.shape[0],
              'To keep for scIB (non NA/multiplet):', eval_cells.sum())
        ...
        for metric in ['PC_regression', 'ASW_batch', 'kBET',
                       'graph_connectivity', 'graph_iLISI',
                      ]:
            metrics_omit[metric]=False
        print('Metrics to compute:',metrics_omit)

        # Run metrics
        print('Data shapes: ref',adata_ref.shape,'latent',latent_adata.shape)
        compute_metrics(adata_full=adata_ref,latent_adata=latent_adata,metrics=metrics,
                        cell_type_col=cell_group, batch_col='study_sample',
                        path_save=path_save,
                        task_name='all_annotatedCells',TMP=TMP,
                        **metrics_omit)
