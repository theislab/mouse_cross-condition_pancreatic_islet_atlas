...
# %%
#adata=h.open_h5ad(path_data+'data_integrated_annotated.h5ad', unique_id2=UID2)
...
# %%
# reset design info from metadata
if True:
    col='design'
    adata.obs.drop(col,inplace=True,axis=1)
    for study in adata.obs.study.unique():
        metadata=pd.read_excel('/lustre/groups/ml01/workspace/karin.hrovatin/data/pancreas/scRNA/scRNA-seq_pancreas_metadata.xlsx',
                              sheet_name=study)
        adata_sub=adata[adata.obs.study==study,:]
        samples=adata_sub.obs.file.unique()
        value_map={sample:metadata.query('sample_name =="'+sample+'"')[col].values[0] for sample in samples}
        adata.obs.loc[adata_sub.obs_names,col]=adata_sub.obs.file.map(value_map)
    del adata_sub
    adata.obs['study_sample_design']=[data.study+'_'+data.file+'_'+data.design
                                      for row,data in adata.obs.iterrows()]
...
# %% [markdown]
# UMAP with main cell types from ref
...
# %% [markdown]
# It seems that general separation of cells in low and high score is comparable across studies. Exceptions are embryo (which are rare among endocrine cells clusters anyways) and VSG and STZ in ins as they contain many diseased cells so Ins+ population is shifted down in its scores.
...
# %%
# Name of CT col
# This was the old integration with less resolved beta cell subtypes
CT_COL_INTEGRATION='cell_type_integrated'
...
keep_obs=['study_sample', 'study', 'file', 'reference', 'size_factors_sample', 
          ...
          'leiden_r0.4', 'design','cell_type_integrated','cell_type_integrated_v1']
...
# %% [markdown]
# # TODO  - anno missing STZ from here on in the endocrine anno code

# %% [markdown]
# ## Beta cells

# %% [markdown]
# ### Subcluster postnatal nonpreliferative beta clusters
...
# %% [markdown]
# Plot markers to detremine if low quality clusters are likely low quality cells or a specific subpopulation
...
# %% [markdown]
# C: Due to different version of scanpy the result here is a bit different
#
# C: There is a cluster of cells thatseem to be comming from activated stellate.
...
# %%
sc.tl.draw_graph(adata_sub, layout='fa')

# %%
sc.pl.draw_graph(adata_sub,color=[score+'_score_scaled' for score in scores],s=30)
...
# %% [markdown]
# #### Add beta and beta-delta cell annotation to whole adata

# %% [markdown]
# Remove doublet clusters and repeat clustering for DE analysis.
...
h.add_category(df=adata.obs,idxs=adata_sub.obs.index[adata_sub.obs[CLUSTERING_SUB]!="8"],
               col='cell_type_integrated',category='beta')
# Add beta-delta cell annotation to whole data
h.add_category(df=adata.obs,idxs=adata_sub.obs.index[adata_sub.obs[CLUSTERING_SUB]=="8"],
               col='cell_type_integrated',category='beta_delta')

# %%
rcParams['figure.figsize']=(6,6)
sc.pl.umap(adata,color='cell_type_integrated',s=2)

# %% [markdown]
# C: Beta-delta cluster may also contian beta-(delta)-alpha cells.

# %%
# This was added latter
adata.obs['cell_type_integrated_v1']=adata.obs['cell_type_integrated'].copy().astype(str)
...
# %% [markdown]
# This was also done for cell_type_integrated beforehand to get the beta data.
...
# %%
#(104030, 13786) # For old beta cells
#(102209, 13999)
adata_beta.shape
...
# %%
#h.save_h5ad(adata=adata_sub,file=path_data+'data_integrated_analysed_beta_v1.h5ad', unique_id2=UID2)
adata_beta.write(path_data+'data_integrated_analysed_beta_v1.h5ad')

# %%
#adata_beta=sc.read(path_data+'data_integrated_analysed_beta_v1.h5ad')

# %% [markdown]
# Savee rawnorm beta data
...
# %%
#adata_rawnorm_beta=sc.read(path_data+'data_rawnorm_integrated_analysed_beta_v1.h5ad')
...
# %%
#adata_beta=sc.read(path_data+'data_integrated_analysed_beta_v1s1.h5ad')
...
h.add_category(df=adata.obs,idxs=adata_sub.obs.index[adata_sub.obs['leiden_r0.5']!="3"],
               col='cell_type_integrated',category='alpha')
# Add gamma cell annotation to whole data
h.add_category(df=adata.obs,idxs=adata_sub.obs.index[adata_sub.obs['leiden_r0.5']=="3"],
               col='cell_type_integrated',category='gamma')

# %%
rcParams['figure.figsize']=(10,10)
sc.pl.umap(adata,color='cell_type_integrated',s=10)

# %% [markdown]
# Add annother correction (different clustering, see above)
...
h.add_category(df=adata.obs,idxs=adata_sub.obs.index[
    ~adata_sub.obs[CLUSTERING_SUB].isin(['12','11','6'])],
               col='cell_type_integrated',category='delta')
# Add beta-delta cell annotation to whole data
...
h.add_category(df=adata.obs,idxs=adata_sub.obs.index[adata_sub.obs[CLUSTERING_SUB]=="6"],
               col='cell_type_integrated',category='alpha_delta')
...
h.add_category(df=adata.obs,idxs=adata_sub.obs.index[adata_sub.obs[CLUSTERING_SUB]=="11"],
               col='cell_type_integrated',category='delta_gamma')
...
# %%
# make pretty UMAPs for HIDA profile
# Change colours and orientations and save the figures
if False:
    adata.uns['cell_type_integrated_colors']=np.array(['#c49c94', #acinar
                                                       '#f5b505', #ductal
                                                       '#17becf', #embryo
                                                       '#f7b6d2', #embryo endocrine
                                                       '#1f77b4', #endocrine proliferative 
                                                       '#9edae5', #endothelial
                                                      '#e377c2', #immune
                                                       '#ff9896', #pericyte
                                                       '#bcbd22', #beta
                                                      ...
                                                      '#e6d705', #alpha_delta
                                                       '#8c564b', #delta_gamma
                                                       '#7f7f7f', #delta
                                                       '#5ba87e']) #fibroblast_related 

    adata_temp=adata.copy()
    adata_temp.obsm['X_umap'][:,0]=adata.obsm['X_umap'][:,0]*-1
    adata_temp.obsm['X_umap'][:,1]=adata.obsm['X_umap'][:,1]*-1
    fig,ax=plt.subplots(figsize=(10,6))
    sc.pl.umap(adata_temp,color='cell_type_integrated',s=2,title='Pancreatic cell types',show=False,ax=ax)
    fig.tight_layout()
    plt.savefig(path_data+'cell_type_inetgrated_prettyHIDA1.pdf') 
    del adata_temp

    adata_temp=adata.copy()
    adata_temp.obsm['X_umap'][:,1]=adata.obsm['X_umap'][:,0]*-1
    adata_temp.obsm['X_umap'][:,0]=adata.obsm['X_umap'][:,1]*-1
    fig,ax=plt.subplots(figsize=(10,6))
    sc.pl.umap(adata_temp,color='cell_type_integrated',s=2,title='Pancreatic cell types',show=False,ax=ax)
    fig.tight_layout()
    plt.savefig(path_data+'cell_type_inetgrated_prettyHIDA2.pdf') 
    del adata_temp
    ...
    sc.pl.umap(adata_temp,color='cell_type_integrated',s=2,title='Pancreatic cell types',show=False,ax=ax)
    fig.tight_layout()
    plt.savefig(path_data+'cell_type_inetgrated_prettyHIDA3.pdf') 
    del adata_temp
    ...
    sc.pl.umap(adata_temp,color='cell_type_integrated',s=2,title='Pancreatic cell types',show=False,ax=ax)
    fig.tight_layout()
    plt.savefig(path_data+'cell_type_inetgrated_prettyHIDA4.pdf') 
    del adata_temp
...
# %%
# reload data
#adata=h.open_h5ad(file=path_data+'data_integrated_analysed.h5ad', unique_id2=UID2)
#adata=sc.read(path_data+'data_integrated_analysed.h5ad')

# %% [markdown]
# ## Annotation consistency
# Consistency of new annotation with previous annotation (my, external) in individual studies.

# %% [markdown]
# #### Integrated annotation vs my per-study annotayion

# %%
rcParams['figure.figsize']=(8,10)
for study in adata.obs.study.unique():
    obs_sub=adata[adata.obs.study==study,:].obs
    if not (obs_sub.cell_type=='NA').all():
        print(study)
        n_cells_previous=obs_sub['cell_type'].value_counts()
        n_cells_integrated=obs_sub['cell_type_integrated_v1'].value_counts()
        display(n_cells_previous)
        confusion=obs_sub.groupby(['cell_type'])[
            'cell_type_integrated_v1'].value_counts(normalize=True).unstack().fillna(0)
        # Sort by ct number
        confusion=confusion.loc[n_cells_previous.index,n_cells_integrated.index]
        fig,ax=plt.subplots()
        sb.heatmap(confusion,xticklabels=True,yticklabels=True,ax=ax)
        display(fig)
        plt.close()

# %% [markdown]
# #### Integration anno vs previous anno

# %%
for study in adata.obs.study.unique():
    obs_sub=adata[adata.obs.study==study,:].obs
    if not (obs_sub['pre_cell_type']=='NA').all():
        print(study)
        n_cells_previous=obs_sub['pre_cell_type'].value_counts()
        n_cells_integrated=obs_sub['cell_type_integrated_v1'].value_counts()
        display(n_cells_previous)
        confusion=obs_sub.groupby(['pre_cell_type'])[
            'cell_type_integrated_v1'].value_counts(normalize=True).unstack().fillna(0)
        # Sort by ct number
        confusion=confusion.loc[n_cells_previous.index,n_cells_integrated.index]
        fig,ax=plt.subplots()
...
# %% [markdown]
# #### My anno vs porevious anno

# %%
for study in adata.obs.study.unique():
    obs_sub=adata[adata.obs.study==study,:].obs
    if not (obs_sub['pre_cell_type']=='NA').all() and not (obs_sub['cell_type']=='NA').all():
        print(study)
        n_cells_previous=obs_sub['pre_cell_type'].value_counts()
        n_cells_integrated=obs_sub['cell_type'].value_counts()
        display(n_cells_previous)
        confusion=obs_sub.groupby(['pre_cell_type'])[
            'cell_type'].value_counts(normalize=True).unstack().fillna(0)
        # Sort by ct number
        confusion=confusion.loc[n_cells_previous.index,n_cells_integrated.index]
        fig,ax=plt.subplots()
        sb.heatmap(confusion,xticklabels=True,yticklabels=True,ax=ax)
...
# %% [markdown]
# C: Only 1 sample of STZ was annotated. Redo confusion with that sample.

# %%
study_sample='STZ_G1'
print(study_sample)
obs_sub=adata[adata.obs.study_sample==study_sample,:].obs
n_cells_previous=obs_sub['pre_cell_type'].value_counts()
n_cells_integrated=obs_sub['cell_type'].value_counts()
display(n_cells_previous)
confusion=obs_sub.groupby(['pre_cell_type'])[
...
# %% [markdown]
# C: Multiplets seem to be better resolved in reference annottion than integrated annotation.
...
# %% [markdown]
# C: Common multiplet types that needed to be resolved in first version of annotation: alpha_beta, alpha_gamma, delta_gamma. However, when looking at embedding these can not be resolved by clustering. There seems to be an alpha_beta cluster, but this seems to come from Fltp_2y (or also other Fltp studies). Corrected in v1.
